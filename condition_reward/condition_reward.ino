...
#define LICK_DEBOUNCE 50       // Lick debounce time
...
const byte ARDUINO_OFF = '0'; // unused
...
void loop() {
  check_lick();  // Always monitor licking
...
void run_trial() {
  ...
  // === TTL pulse for trial start ===
  digitalWrite(TRIAL_ON, HIGH);
  if (rand_choice == 0) {
    delayWithLickCheck(TTL_TONE1_DURATION);
  } else {
    delayWithLickCheck(TTL_TONE2_DURATION);
  }
  digitalWrite(TRIAL_ON, LOW);
  ...
  // === Post-trial interval with lick monitoring ===
  int POST_TRIAL_DELAY = random(4000, 6001);
  delayWithLickCheck(POST_TRIAL_DELAY);
}
...
// === Custom delay that continues licking detection ===
void delayWithLickCheck(unsigned long duration) {
  unsigned long start = millis();
  while (millis() - start < duration) {
    check_lick();
    delay(1);  // Prevent CPU lockup
  }
}
